...
export async function verifyOnSolana(inspection: {
    id: string; report_number: string; machine_model: string; status: string
    risk_score: number; nogo_count: number; caution_count: number; report_hash?: string
}, privateKeyB58: string): Promise<{ signature: string; explorer_url: string; verified_at: string }> {
    ...
    const payload = JSON.stringify({
        ...
        nogo: inspection.nogo_count,
        hash: inspection.report_hash?.slice(0, 16),
        ts: Date.now()
    })
